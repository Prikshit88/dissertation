...
# Train One-Class SVM for each procedure (column)
ph_2_data2 = pd.DataFrame(index=ph_2_data1.index, columns=ph_2_data1.columns)
for col in ph_2_data1.columns:
    svm = OneClassSVM(nu=0.05, kernel='rbf')
    svm.fit(scaled_data[:, [ph_2_data1.columns.get_loc(col)]])
    ph_2_data2[col] = svm.predict(scaled_data[:, [ph_2_data1.columns.get_loc(col)]]) == -1  # True for outlier
...
output.to_csv(r'C:\Users\ptush\OneDrive\Documents\Dessertation\output\Output_SVM.csv')
